...
export default function PosPage() {
  return (
    <Guard allow={["admin", "cashier"]}>
      {(session) => <PosInner cashierId={session.userId} role={session.role} name={session.displayName} />}
    </Guard>
  );
}

function PosInner({ cashierId, role, name }: { cashierId: string; role: string; name: string }) {
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        {loading ? <Spinner label="Loading register…" /> :
         error ? <Banner kind="error">{error} — check your Google Sheets connection.</Banner> : (
          <>
            <div className="mb-5 flex gap-1 rounded-xl border border-border bg-surface p-1 w-fit">
              {(["register", "booking"] as const).map((t) => (
                <button key={t} onClick={() => setTab(t)}
                  className={`rounded-lg px-4 py-1.5 text-sm font-medium capitalize transition-colors ${tab === t ? "bg-primary text-primaryText" : "text-textMuted hover:text-text"}`}>
                  {t}
                </button>
              ))}
            </div>
            {tab === "register"
              ? <Register services={services} attendants={attendants} cashierId={cashierId} />
...
function Register({ services, attendants, cashierId }: { services: Service[]; attendants: Attendant[]; cashierId: string }) {
  ...
  useEffect(() => { if (!activeCat && categories[0]) setActiveCat(categories[0]); }, [categories, activeCat]);
  ...
  function addService(s: Service) { setCart((c) => [...c, { service: s, attendantIds: [] }]); }
  ...
  function toggleAttendant(itemIdx: number, attId: string) {
    setCart((c) => c.map((item, idx) => {
      ...
      return { ...item, attendantIds: has ? item.attendantIds.filter((x) => x !== attId) : [...item.attendantIds, attId] };
    }));
  }
  ...
  async function checkout() {
    ...
    if (cart.some((i) => i.attendantIds.length === 0)) return setErr("Assign an attendant to every service.");
    setSaving(true);
    try {
      const sale = {
        ...
        customer, customerPhone: phone, customerEmail: email, vehicle,
        carPhotoUrl, signatureUrl, cashierId, status: "paid" as const,
      };
      await api.checkout(sale);
      setDone(true);
      setCart([]); setTip(0); setTipAttendant(""); setCustomer(""); setPhone(""); setEmail("");
      setVehicle(""); setCarPhotoUrl(""); setSignatureUrl("");
    } catch (e) { setErr(e instanceof Error ? e.message : "Checkout failed"); }
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_400px]">
      <div>
        <div className="mb-4 flex flex-wrap gap-2">
          {categories.map((c) => (
            <button key={c} onClick={() => setActiveCat(c)} className={`chip ${activeCat === c ? "glow-active text-text" : ""}`}>{c}</button>
          ))}
        </div>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {services.filter((s) => s.category === activeCat).map((s) => (
            <button key={s.id} onClick={() => addService(s)}
              ...
              <div className="mt-1 flex items-center justify-between">
                <span className="text-base font-semibold" style={{ color: "var(--primary)" }}>{money(s.price)}</span>
      ...
      <aside className="panel flex h-fit flex-col gap-4 p-5 lg:sticky lg:top-4">
            ...
            <div key={i} className="rounded-lg border border-border bg-bg p-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm" style={{ color: "var(--primary)" }}>{money(item.service.price)}</span>
              ...
              <div className="mt-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {attendants.map((a) => (
                    <button key={a.id} onClick={() => toggleAttendant(i, a.id)}
                      className={`chip ${item.attendantIds.includes(a.id) ? "glow-active text-text" : ""}`}>{a.name}</button>
                  ))}
                </div>
              </div>
            </div>
        ...
        <PhotoCapture label="Car photo (documentation)" value={carPhotoUrl} onChange={setCarPhotoUrl} />

        <div>
          <span className="label">Tip</span>
          <div className="flex gap-2">
            <input type="number" min={0} className="input" value={tip || ""} placeholder="0.00"
      ...
      <Modal open={done} onClose={() => setDone(false)} title="Payment recorded">
        <p className="text-sm text-textMuted">
          Sale saved. Inventory was automatically deducted based on the services performed.
          Commission and tips are reflected in the attendant and admin views.
        </p>
        <button className="btn-primary mt-4 w-full" onClick={() => setDone(false)}>New ticket</button>
      </Modal>
    </div>
  );
}
...
function BookingCalendar({ services, attendants }: { services: Service[]; attendants: Attendant[] }) {
  ...
  async function load() {
    setLoading(true);
    try { setBookings(await api.bookings()); }
    catch (e) { setErr(e instanceof Error ? e.message : "Failed to load bookings"); }
    finally { setLoading(false); }
  }
  useEffect(() => { load(); }, []);
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
      <div className="panel p-5">
        <SectionTitle action={<input type="date" className="input w-auto" value={date} onChange={(e) => setDate(e.target.value)} />}>
          Day agenda
        </SectionTitle>
        {loading ? <Spinner /> : (
            ...
            {hours.map((h) => {
              ...
              return (
                <div key={h} className="flex gap-3 py-2">
                  <span className="w-14 shrink-0 pt-1 text-xs text-textMuted">{h % 12 === 0 ? 12 : h % 12}:00 {h < 12 ? "AM" : "PM"}</span>
      ...
      <NewBookingModal open={open} onClose={() => setOpen(false)} services={services} attendants={attendants}
        defaultDate={date} onSaved={() => { setOpen(false); load(); }} />
    </div>
  );
}

function NewBookingModal({ open, onClose, services, attendants, defaultDate, onSaved }: {
  open: boolean; onClose: () => void; services: Service[]; attendants: Attendant[]; defaultDate: string; onSaved: () => void;
}) {
  const [customer, setCustomer] = useState("");
  const [phone, setPhone] = useState("");
  const [vehicle, setVehicle] = useState("");
  ...
  async function save() {
    ...
    try {
      ...
      await api.append(TABS.bookings, {
        id: uid("B-"), datetime: iso, customer, phone, vehicle,
        serviceIds: serviceIds.join("|"), attendantId, status: "booked", notes: "",
      });
      onSaved(); setCustomer(""); setPhone(""); setVehicle(""); setServiceIds([]); setAttendantId("");
  ...
  return (
    <Modal open={open} onClose={onClose} title="New booking">
      <div className="space-y-3">
        ...
        {err && <Banner kind="error">{err}</Banner>}
        <button className="btn-primary w-full" disabled={saving} onClick={save}>{saving ? "Saving…" : "Save booking"}</button>
      </div>
    </Modal>
  );
}
